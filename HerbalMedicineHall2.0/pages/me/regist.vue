<template>
	<view>
		...
		<u-modal v-model="show2" :content="content2" :mask-close-able="true"></u-modal>
		<u-toast ref="uToast" />
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			show_modal:function(){
				...
				if(f){
					let registUser={
							"name":this.nickname==''?"defultname":this.nickname,
							"password":this.password,
							"phoneNumber":this.phoneNumber,
							"id":this.account
						}
					getApp().globalData.registUser=registUser;
					// console.log(getApp().globalData.registUser);
					let _this=this;
					uni.request({
						...
						success(data) {
							...
							if(data.data==0){
								...
								_this.content2="该账号已存在!";
								_this.accountErrorMessage="该账号已存在!";
								_this.show2=true;
							}
						}
					});
			...
			regist:function(){
				...
				uni.request({
					...
					success(data) {
						...
						if(data.data==0){
							uni.request({
								url:getApp().globalData.requestAddUser,
								data:{
									user:getApp().globalData.registUser
								},
								success(data) {
									// console.log(data);
							...
							_this.content2="该手机号已存在!";
							_this.phoneNumberErrorMessage="该手机号已存在!";
							_this.show2=true;
						}
					}
				});
